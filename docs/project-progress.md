...
- 🔎 Root causes found (2026-06-11) why a granted PAGE never appears on the device:
  ...
  4. The sync cmd ack `ok:true` only means "queued"; install success/failure is only visible in the serial log (`sync:` tag). **Update 2026-06-14: installed packages now join the native swipe rotation** (no longer replace the whole UI) via lazy load/unload on swipe — see "Multi-page lazy-swap on device" below.
...
## Multi-page lazy-swap on device — VERIFIED on hardware (2026-06-14)
- ✅ **All installed pages now share the swipe rotation** (was: only the single sync "active" package showed). `setup_pages_from_cfg()` keeps every `settings.pages` slug whose package resolves on SD via the new `sync_manager_installed_dir_for_slug()` (scans `/sd/packages/<id>/current.txt`), capped at **5** (`ROTATION_MAX`; the UserApp curates the list).
- ✅ **Lazy load/unload — only one package loaded at a time** (so no extra memory pressure). Swiping to a package page that isn't loaded parks the display on a spinner and asks app_main (`home_ui_set_package_activator`) to swap; the swap runs on the **existing sync worker** (`sync_manager_request_nav` → `do_page_swap`: `wasm_engine_unload_all` → `ui_renderer_load_dir` → subscribe → `wasm_engine_load_modules` → deliver `settings.<slug>` → `home_ui_package_loaded` adopts the screen). Reusing the sync worker means **no new task stack** → MQTT no longer OOMs.
- ✅ **Per-page lifecycle**: crypto's Binance poll starts on enter and **stops on leave**; slideshow timer pauses/resumes — so off-screen pages don't do work (the user's "one page at a time" rule).
- ✅ **Hardware-proven** on `ccp-983daee91478`: `pages` shows all 5 (clock,crypto,slideshow,weather,profile); `goto weather` swaps in weather's real tree (7 widgets + GIF, Bangkok 31°C, Humidity 68%); `goto profile` swaps in profile (HAL FINNEY, live clock); `goto crypto` starts polling (`quote BTCUSDT …`) and `goto clock` stops it; heap stays ~11–17KB internal free across swaps; MQTT connects every boot.
- ⚠️ `home_ui_package_loaded` waits on `display_engine_lock(0)` (forever, like `home_ui_reload`) — a short timeout lost to the spinner + freshly-started wasm and stranded the swap. The swap is queued/async so the LVGL task never blocks (would trip the 10s task-WDT).
