...
| #5 AppUser แก้ field ของ Builder Profile | ✅ แก้แล้ว (2026-06-14) | เพิ่ม card "Profile page" ในแอป (`settings_pages.dart` `ProfileSettings`) ให้แก้ `nickname/role/name_color/show` → เขียน `config.profile.*` → device รับเป็น `settings.profile.*`. **เพิ่ม page-curation ฝั่งแอปด้วย**: `SystemSettings` แสดงหน้าที่ entitled ทั้งหมด (native + PAGE-kind จาก catalog∩entitlements), จำกัด ≤5, ลาก reorder = ลำดับ swipe (`DeviceController.availablePages/maxPages/reorderPages`). `flutter analyze` ผ่าน (เหลือแค่ info เดิม). _ยังเหลือ: generic SchemaForm mirror สำหรับ Builder page อื่นๆ (เช่น weather) ต้องมี endpoint ดึง settings_schema ราย package จาก Hub — chunk ย่อยถัดไป._ |
