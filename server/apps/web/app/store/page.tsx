  ...
  return (
    <main className="p-6">
      ...
      <p className="text-sm text-[var(--ccp-muted)] mb-5">
        Extra pages and feature add-ons customers can buy. Admin grants activate purchases on each display.
      </p>
        ...
        {items.map((it) => (
          <div key={it.slug} className="card p-4 flex flex-col gap-2">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="text-[10px] uppercase tracking-wide text-[var(--ccp-muted)]">{it.slug}</div>
            </div>
      ...
      {me?.isAdmin && (
        <section>
          <h2 className="text-lg font-semibold mb-2">Manage store items</h2>
          {managed.length === 0 ? (
            <div className="card p-5 text-sm text-[var(--ccp-muted)]">
              No store items yet. Publish a layout from the Builder or seed the
              catalog, then set its price and publish flag here.
            </div>
          ) : (
                ...
                <tbody>
                  {managed.map((m) => (
                    <tr key={m.id} className="border-b border-[var(--ccp-border)]">
                      ...
                      <td className="p-3 text-xs text-[var(--ccp-muted)]">{m.slug}</td>
