...
/*
 * esp_lcd_panel_disp_on_off() passes on_off==true meaning "display ON".
 * The original vendor file treated this flag as "off", which inverts the
 * behaviour under the modern esp_lcd API and leaves the panel in DISPOFF
 * (black screen, since the board init table contains no DISPON either) —
 * fixed to standard semantics here.
 */
